...
void eeprom_write(char *filename)
{
	...
	for(addr=0; addr<size;addr+=2)
	{
		...
		if (evalue != value) {
			...
			if (!dev.ops->eeprom_read16(&dev, addr, &tmp)) {
				printf("\n**** Error reading EEPROM back the WORD just written ****\n");
			}
			if (value == tmp)
				printf("x");
			else
				printf("\n**** Write verify error: Addr %04x, wrote %2x, read %2x ****", addr, value, tmp);
		} else {
			printf(".");
		}
		if (0x7E ==(addr & 0x7F)) printf("]\n");
...
int main(int argc, char** argv)
{
	...
	while (1) {
		c = getopt_long(argc, argv, "r:wlLF:d:mcni:o:bhpsID:", long_options, NULL);
		if (c == -1)
			break;
		switch(c) {
			case 'l':
			...
			case 'D':
				...
				break;
			case 'h':
				die("EEPROM reader/writer for Atheros Wi-Fi cards\n\n"
					"Usage:\n"
					"\t%s [-d device [-m] [-c] | -n] [-I] [-i filename ] [-o filename [-b] ] [-P] [-p] [-r Reg Domain] [-D debug_level]\n"
					"\t%s -l\n"
					"\t%s -h\n\n"
					"Options:\n"
					...
					"\t-r <hex reg. Domain> | --patch_rd\t"
					"patch device eeprom with a new Reg. Domain\n"
					"\t-I | --init\t\t\t\t"
					"init device (useful if driver didn't do it)\n"
					"\t-s | --show\t\t\t\t"
	...
	if (debug)
		printf("Supported ops: %s%s%s%s%s\n",
			...
			dev.ops->eeprom_patch_rd ? " patch_rd" : ""
		);
	...
	if(!ifname && !ofname && !patch11n && !parse && !patch_rd)
	...
	if (ifname && dev.ops->eeprom_writable) {
		eeprom_write(ifname);
	} else if (!dev.ops->eeprom_writable)
                die("**** Read-only eeprom! ****\n");

	if (patch11n && dev.ops->eeprom_writable)
		dev.ops->eeprom_patch11n(&dev);

	if (patch_rd && dev.ops->eeprom_writable) {
		printf("\n You entered %04x Reg Domain\n", regDmn);
		dev.ops->eeprom_patch_rd(&dev, regDmn);
	}
