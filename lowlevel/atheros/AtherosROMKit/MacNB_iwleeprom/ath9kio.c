...
// sending data
/*
	printf("\n**** [EEPROM Offset (addr>>1) S)] Value = [%04x %04x %04x] %02x\n",
		  AR5416_EEPROM_OFFSET,   (addr >> 1), AR5416_EEPROM_S, value);
	printf("\n**** [EEPROM Offset + (addr>>1) << S)] Value = [%04x] %02x\n",
		  AR5416_EEPROM_OFFSET + ((addr >> 1) << AR5416_EEPROM_S), value);
	printf("\n**** [EEPROM Offset + (addr>>1) << S)] Value = [%04x] %02x\n",
		  AR5416_EEPROM_OFFSET + addr, value);

	PCI_OUT16(AR5416_EEPROM_OFFSET + addr, value);
*/
...
// waiting...
	for(i = WAIT_TIMEOUT; i; i--) {
		...
		data = PCI_IN32(AR_EEPROM_STATUS_DATA);
		if ( 0 == (data & (AR_EEPROM_STATUS_DATA_BUSY | AR_EEPROM_STATUS_DATA_PROT_ACCESS))) {
//			tmp = data & AR_EEPROM_STATUS_DATA_VAL;
//			printf("\n Value read after write: %2x\n", tmp);
			break;
		}
	}
...
static void ath9k_eeprom_patch_rd(struct pcidev *dev, uint16_t new_regDmn)
{
	uint16_t
		current_regDmn;
	...
	printf("Patching Reg Domain in card EEPROM...\n");

	dev->ops->eeprom_read16(dev, short_eeprom_base + 8, &current_regDmn);

	printf("Current Reg. domain : %04x\n", current_regDmn);

	dev->ops->eeprom_write16(dev, short_eeprom_base + 8, new_regDmn);

	// Verify the write
	dev->ops->eeprom_read16(dev, short_eeprom_base + 8, &current_regDmn);
	printf("    New Reg. domain : %04x\n", current_regDmn);
	if (current_regDmn != new_regDmn) {
		printf("Failed to updated the Reg. Domain\n");
		return;
	}
...
struct io_driver io_ath9k = {
	...
	.eeprom_patch_rd = &ath9k_eeprom_patch_rd,
