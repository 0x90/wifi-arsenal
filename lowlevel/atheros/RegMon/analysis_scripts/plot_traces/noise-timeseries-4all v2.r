...
#Arbeitspfad setzen
setwd("/Volumes/cracker/data/nfs/thomas/experiments/indoor-noise-17.07.2011-nosync-noise0-cca62-ofdm1-ch6/data/merged-trace/aggregation")

#Einlesen
noise_ts <- read.csv(file = "tel-16-1-10-sec-aggregation.csv", header = T , sep = ",")
...
mf_labeller <- function(var, value){
  ...
  if (var=="sex") { 
    value[value=="high"] <- "CS using energie detection & preamble correlation"
    value[value=="low"]   <- "CS using only energie detection"
  }
  return(value)
}
...
#boxplots over time
print(
  ggplot(data = noise_ts, aes(x = timestamp/3600, y = median)) +
    #geom_point(aes(y = median),size=0.9, alpha=0.2, colour = "blue") +
    #geom_smooth(aes(x = timestamp/3600, y = max), size=0.9, alpha=0.2, colour = "red") +
    #geom_smooth(aes(x = timestamp/3600, y = min), size=0.9, alpha=0.2, colour = "green") +
    #coord_cartesian(ylim = c(-105,-95)) 
    geom_boxplot(aes(group = round_any(timestamp, 1800, floor)), outlier.colour = "red", outlier.size = 1, alpha=0.3) +
    ...
    labels=c("22:00", "24:00", "2:00", "4:00", "6:00", "8:00", "10:00", "12:00", "14:00", "16:00", "18:00", "20:00",
    ...
    opts(axis.text.y = theme_text(hjust = 1, size = 8, colour = "black"),
              ...
              title ="Noisefloor distribution in 36h on channel 6 - BOWL indoor",
      ...
      facet_wrap(~ node, ncol=2, scales = "free_y")
)
