...
wnic_read_logger::wnic_read_logger(wnic_sptr wnic) :
   wnic_wrapper(wnic),
   dl_()
{
   ...
   string file_name(wnic->name() + "-read.pcap");
...
wnic_read_logger::~wnic_read_logger()
{
...
buffer_sptr
wnic_read_logger::read()
{
   buffer_sptr b(wnic_->read());
   if(b) {
      size_t n = 0;
