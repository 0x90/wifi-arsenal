...
if [ $# -lt 3 ]; then
	 echo "usage: plot-some-time.sh file.data field [field*]"	1>&2
	 exit 1
fi
...
if [ -s "$t" ]; then
	 ...
	 for f in $*; do
		  if [ "${f/-*/}" == "Airtime" ]; then
				s="${s}${d}\"${t}\" using 1:${n} with lp title \"${f}\""
		  elif [ "${axis[$f]}" == "" ]; then
				s="${s}${d}\"${t}\" using 1:(rate2time(\$${n})) with lp title \"${f}\""
		  else
