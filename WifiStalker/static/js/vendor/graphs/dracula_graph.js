    ...
    drawNode: function(node) {
        ...
        if(!node.render) {
            node.render = function(r, node) {
                ...
                var stroke=node.stroke, fill=node.fill;
                if (!fill)
                ...
                switch (node.type) {
                case 'rect':
                    obj = r.rect(0, 0, 20, 20).attr({fill: fill, stroke: stroke, "stroke-width": 2});
                    txt = r.text(10, 30, node.label || node.id);
                    break;
                case 'smallellipse':
                    obj = r.ellipse(0, 0, 10, 10).attr({fill: fill, stroke: stroke, "stroke-width": 2});
                    txt = r.text(0, 20, node.label || node.id);
                    break;
                case 'ellipse':
                default:
                    obj = r.ellipse(0, 0, 20, 20).attr({fill: fill, stroke: stroke, "stroke-width": 3});
                    txt = r.text(0, 30, node.label || node.id);
                    break;
                }
                /* set DOM node ID - FIXME: This should still be node.id */
                obj.node.id = node.id;
                shape = r.set().
                    push(obj).
                    push(txt);
