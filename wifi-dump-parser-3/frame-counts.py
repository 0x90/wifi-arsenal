...
# To count the types of frames in the dataset; from transmitted to received or FCS correct/failed
import os,sys,re
import gzip
import struct 
...
rx_data_fc_correct= 0
rx_data_fc_incorrect =0
tx_data_fc_incorrect =0
tx_data_fc_correct =0

tx_ctrl_fc_incorrect =0 
tx_ctrl_fc_correct =0
rx_ctrl_fc_incorrect =0
rx_ctrl_fc_correct =0

rx_mgmt_fc_incorrect =0
tx_mgmt_fc_incorrect =0
rx_mgmt_fc_correct =0
tx_mgmt_fc_correct =0
...
for data_f_name_list in filename_list : #data_fs :    
    ...
    for idx in xrange(0,len(correct_data_frames)-DATA_STRUCT_SIZE ,DATA_STRUCT_SIZE ):	
        ...
        if success:
            ...
            if radiotap_len == RADIOTAP_RX_LEN:
                rx_data_fc_correct= rx_data_fc_correct + 1
            elif radiotap_len ==RADIOTAP_TX_LEN :
                tx_data_fc_correct= tx_data_fc_correct + 1
            else :
    ...
    for idx in xrange(0,len(err_data_frames)-DATA_ERR_STRUCT_SIZE,DATA_ERR_STRUCT_SIZE ):	
        ...
        if success:
            ...
            temp.insert(0,tsf)
            if radiotap_len == RADIOTAP_RX_LEN:                
                rx_data_fc_incorrect= rx_data_fc_incorrect + 1
            elif radiotap_len ==RADIOTAP_TX_LEN :
                tx_data_fc_incorrect= tx_data_fc_incorrect + 1
            else :
    ...
    for idx in xrange(0,len(beacon_mgmt_frames)-MGMT_BEACON_STRUCT_SIZE ,MGMT_BEACON_STRUCT_SIZE ):
        ...
        if success==1 :
            if radiotap_len == RADIOTAP_RX_LEN:
                rx_mgmt_fc_correct= rx_mgmt_fc_correct + 1
            elif radiotap_len == RADIOTAP_TX_LEN:
                tx_mgmt_fc_correct= tx_mgmt_fc_correct + 1
        else :
    ...
    for idx in xrange(0,len(err_mgmt_frames)-MGMT_ERR_STRUCT_SIZE,MGMT_ERR_STRUCT_SIZE ):
        ...
        if success==1 :
            if radiotap_len == RADIOTAP_RX_LEN:
        	rx_mgmt_fc_incorrect= rx_mgmt_fc_incorrect + 1
            elif radiotap_len == RADIOTAP_TX_LEN:
        	tx_mgmt_fc_incorrect= tx_mgmt_fc_incorrect + 1
    ...
    for idx in xrange(0,len(correct_ctrl_frames)-CTRL_STRUCT_SIZE ,CTRL_STRUCT_SIZE ):			
        ...
        if success==1 :
            if radiotap_len == RADIOTAP_RX_LEN:
        	rx_ctrl_fc_correct= rx_ctrl_fc_correct + 1
            elif radiotap_len ==RADIOTAP_TX_LEN :
        	tx_ctrl_fc_correct= tx_ctrl_fc_correct + 1
        else :
            print "success denied"
    ...
    for idx in xrange(0,len(err_ctrl_frames)-CTRL_ERR_STRUCT_SIZE,CTRL_ERR_STRUCT_SIZE):			
        ...
        if success ==1:
            if radiotap_len == RADIOTAP_RX_LEN:
        	rx_ctrl_fc_incorrect= rx_ctrl_fc_incorrect + 1
            elif radiotap_len ==RADIOTAP_TX_LEN :
        	tx_ctrl_fc_incorrect= tx_ctrl_fc_incorrect + 1
        else :
            print "success denied"
...
print "Statistics "
print "Number of Mgmt frames ",  rx_mgmt_fc_incorrect + rx_mgmt_fc_correct+tx_mgmt_fc_incorrect + tx_mgmt_fc_correct
print "Number of Data frames ", tx_data_fc_incorrect+ tx_data_fc_correct + rx_data_fc_incorrect+ rx_data_fc_correct
print "Number of Ctrl frames ",  tx_ctrl_fc_incorrect +  tx_ctrl_fc_correct + rx_ctrl_fc_incorrect + rx_ctrl_fc_correct
print "Percentage of Mgmt frames ",  (  rx_mgmt_fc_incorrect + rx_mgmt_fc_correct+tx_mgmt_fc_incorrect + tx_mgmt_fc_correct) *100.0/(rx_mgmt_fc_incorrect+ rx_mgmt_fc_correct+tx_mgmt_fc_correct + tx_mgmt_fc_incorrect + tx_data_fc_correct+ tx_data_fc_incorrect+rx_data_fc_correct + rx_data_fc_incorrect + rx_ctrl_fc_correct + rx_ctrl_fc_incorrect + tx_ctrl_fc_correct+ tx_ctrl_fc_incorrect)
