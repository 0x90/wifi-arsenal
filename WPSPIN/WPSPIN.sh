...
cat scan.txt | sed '1,6d'  > wash_scan.txt                                     # | sed '$d'
...
echo -e "$magenta
    ...
    $amarillo www.crack-wifi.com     www.lampiweb.com    www.auditoriaswireless.net$colorbase
...
Default PIN will be displayed:
  ...
  - in$orange orange$blanco if the device is unknown or unsupported $colorbase
         ...
         $magenta               < CTRL + C > TO STOP THE SCAN $colorbase ")
...
STOP_REAVER=$(echo -e " $blanco                      < CTRL + C > to stop the attack $colorbase "
 )
...
$magenta                    < CTRL + C > para parrar el esacneo   $colorbase") 
...
STOP_REAVER=$(echo -e " $blanco                 < CTRL + C > PARA PARRAR EL ATAQUE $colorbase "
 )
...
Le PIN par défaut proposé apparait:
 ...
 $magenta                   PRESSEZ < CTRL +C > POUR ARRÊTER LE SACN $colorbase") 
...
STOP_REAVER=$(echo -e " $blanco                 < CTRL + C > pour arrêter l'attaque $colorbase "
 )
...
$blanco          Il est recommandé de choisir l'option 3 (changer d'interface)$colorbase "
 )
