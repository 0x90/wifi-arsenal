					  ...
					  size_t crypto_tlv_len)
{
	...
	// Spoof: don't care about cryptobinding ;)----------------------------
	...
	wpa_hexdump(MSG_INFO, "spoof: Received Compound_MAC (not checking)", pos, SHA1_MAC_LEN);
	os_memcpy(compound_mac_spoof, pos, SHA1_MAC_LEN);
	// --------------------------------------------------------------------
