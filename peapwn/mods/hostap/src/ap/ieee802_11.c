...
// Checks association information elements (ies)
			 ...
			 int reassoc)
{
 ...
 fail:
 	 // Disconnect open and wpa2 networks immediately
 	if(hapd->conf->ieee802_1x == 0 && hapd->conf->safespoof == 1) { // TODO: make option to disconnect users that already tried to connect?
 		ap_sta_disconnect(hapd, sta, sta->addr, WLAN_REASON_DISASSOC_AP_BUSY);
		hostapd_drv_sta_disassoc(hapd, sta->addr, WLAN_REASON_DISASSOC_AP_BUSY);
		wpa_printf(MSG_INFO, "Inhibited association response, deauth and disassoc because safe spoofing is enabled.");
 	} else {
		     ...
		     struct hostapd_frame_info *fi)
{
	...
	// Buf contains the raw data and gets cast to mgmt struct here
	...
	if (stype == WLAN_FC_STYPE_PROBE_REQ) {
		if (hapd->conf->cycle_probe_response_types) {
			// Do probe response as open network, 802.1X network and WPA2-PSK network
			spoof_cycle_ie(hapd);
			handle_probe_req(hapd, mgmt, len, fi->ssi_signal);
		} else { // Normal probe response
			handle_probe_req(hapd, mgmt, len, fi->ssi_signal);
		}
		return;
	}
